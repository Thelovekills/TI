...
var db = {
    ...
    deleteCryptoByCanti : function (cantidad) {
      var index;
      //Buscamos el indice del alumno
      for (index = 0; index < this.cryptos.length; index++) {
        if(this.cryptos[index].cantidad == cantidad )
        break;
      }
      
      //Si se encontro el indice se elimina
      if(index<db.cryptos.length){
        delete db.cryptos[index];
        db.cryptos.splice(index, 1);
        db.saveCryptos();
      }
    },
...
app.route("/cryptos")
  ...
  .delete((req,res)=> {
    ...
    console.log("Objeto post recibido");
    console.log(crypto);
    db.deleteCrytpoByCanti( crypto.cantidad );
    console.log(db.cryptos);
    res.json({'status' : 'OK'})
  });

app.get('/cryptos/:cantidad', (req, res) => {
  db.initDB();
